...
function fetch_all_posts()
{
	...
	if ($result->num_rows > 0) {
		while ($row = $result->fetch_assoc()) {
			...
			echo "<div class='card'>
				  <img class='card-img-top' src='" . $user['profile_image'] . "' alt=''><i><b>" . $user['first_name'] . " " . $user['last_name'] . "</b></i></p>
				  <div class='card-body'>
		            <p class='card-text'>" . $row['content'] . " </p>
				    <p class='card-text'><i>Date: <b>" . $row['created_time'] . "</b></i></p>
	              	<div class='likes'>Likes: <b id='likes_" . $row['id'] . "'>" . $row['likes'] . "</b>
	              		 <button onclick='like_post(this)' data-post_id='" . $row['id'] . "'>LIKE</button>
	               	</div>
	               	</div>
	           </div>";
		}
